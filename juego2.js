* This file, related code, assets, styling and indicia are Copyright (c) Friv.com 2006-2020. Unauthorised reproduction is prohibited. All rights reserved. */
...
var isMega = false;
if (window.location.hostname.indexOf("frivmega") > -1 ) {
    isMega = true;
}
...
if (isMega) {
    game.title = game.title + "FRIVMEGA ";
}
...
if (isSchool) {
    ...
    if (os == "Desktop") {
        gtag('config', 'UA-124684579-6', { //windows and other users (x% sample, no pages, events only) TYPICAL SAMPLE RATE: 5
            'send_page_view': false,
            'sample_rate': 5
        });
    } else {
...
setTimeout(function () {
    gtag('event', ("5 Mins : " + game.title), {
        'event_category': 'GameX Play',
        'event_label': ('GameX : 5 Mins : ' + categoryArray[0])
    });
}, 300000);
...
setTimeout(function () {
    gtag('event', ("3 Mins : " + game.title), {
        'event_category': 'Game Play Quick View',
